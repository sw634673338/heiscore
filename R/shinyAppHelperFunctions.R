...
variableList_MCMC <- list( "Energy (Kilocalories)" = "KCAL",
                           "Whole Fruits" = "FWHOLEFRT",
                           "Fruit Juice" = "F_JUICE",
                           'Non-Dark Green Vegetables' = 'VNONDRKGR',
                           'Dark Green Vegetables' = 'V_DRKGR',
                           'Legumes (as a Vegetable)' = 'V_LEGUMES',
                           'Whole Grains' = 'G_WHOLE',
                           'Total Dairy' = 'D_TOTAL',
                           'Meat, Poultry, and Eggs' = 'PF_MPE',
                           'Seafood, Soy, Nuts, and Seeds' = 'PF_SSNS',
                           'Legumes (as a Protein Food)' = 'PF_LEGUMES',
                           'Total Mono and Polyunsaturated Fatty Acids' = 'MONOPOLY',
                           'Refined Grains' = 'G_REFINED',
                           'Sodium' = 'TSODI',
                           'Added Sugars' = 'ADD_SUGARS',
                           'Saturated Fat' = 'TSFAT'
...
xVarMax <- function(variable, all_datasets){
  all_maxes <- c()
  for(years in all_datasets){
    current_year <- years %>%
      dplyr::select(dplyr::contains(variable))

    all_maxes <- c(all_maxes, (max(current_year[,1], current_year[,2], na.rm = T)))
